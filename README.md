...
| 功能模組 | 說明 |
|---|---|
| **雙模型分類** | CLIP ViT-B/32（zero-shot，免訓練）＋ 自訓 CNN（4-block, 從頭訓練, Val Acc 90.51%） |
...
│   ├── custom_cnn_model.py              # DisasterCNN_v1 架構（4-block）
│   ├── custom_cnn_classifier.py         # 自訓 CNN 推論模組
│   ├── custom_cnn.pth                   # 訓練好的權重（從 Kaggle 下載，未進 git）
│   ├── custom_cnn_classes.json          # 類別對照（從 Kaggle 下載，未進 git）
│   ├── resnet_baseline.py               # ResNet50 Linear Probe 推論（保留，未來 Partial FT 用）
│   └── train_resnet.py                  # ResNet50 訓練腳本（保留）
│
├── rag/                       # 檢索增強生成
...
| 英文 (CLIP 輸出 + 自訓 CNN folder name) | 中文 (顯示用) |
|---|---|
| Cyclone | 颱風或強風災損 |
| Earthquake | 地震或建築損壞 |
| Flood | 淹水 |
...
> v1 採用 4 類結構，對齊 Kaggle [`mikolajbabula/disaster-images-dataset-cnn-model`](https://www.kaggle.com/datasets/mikolajbabula/disaster-images-dataset-cnn-model) 的 schema。沒有 Non_Damage（無災害）與 Landslide（土石流）— 任何非災害照片仍會被分到 4 類其中之一；考慮使用低信心度標記輔助辨識。
...
AdaptiveAvgPool(1) + Flatten + Dropout(0.3) + Linear(256, 4)
```
...
| 參數量 | 390,404 |
| Val Acc | **90.51%**（4 類） |
| Per-class recall | Cyclone 91.6% / Earthquake 95.2% / Flood **67.2%** / Wildfire 93.4% |
...
- Add data → 搜尋 `mikolajbabula/disaster-images-dataset-cnn-model`（4 類版本）
- 確認 Cell 4 的 `DATA_DIR` 路徑指向實際目錄

### 3. Run All

- 約 30-60 分鐘跑完 4 個變體（v1 baseline + v2/v3/v4 ablation）
- v1 預期 Val Acc ≥ 60%（實測 90.51%）
- 詳細訓練設定與超參數可看 notebook 內 Cell 4
...
**v1.1** · CLIP + 自訓 CNN (DisasterCNN_v1) + RAG · 2026
