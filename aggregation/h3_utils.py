...
def latlng_to_h3_cell(lat: float, lng: float, resolution: int = DEFAULT_RESOLUTION) -> str:
    ...
    """
    if h3 is None:
        raise RuntimeError("缺少 h3 套件，無法使用 GPS/H3 聚合。請安裝 requirements.txt。")
...
def h3_cell_to_latlng(cell: str) -> tuple[float, float]:
    ...
    if h3 is None:
        raise RuntimeError("缺少 h3 套件，無法轉換 H3 cell。")
...
def cell_boundary(cell: str) -> list[tuple[float, float]]:
    ...
    if h3 is None:
        raise RuntimeError("缺少 h3 套件，無法取得 H3 邊界。")
...
def cell_to_parent(cell: str, target_resolution: int) -> str:
    ...
    if h3 is None:
        raise RuntimeError("缺少 h3 套件，無法聚合 H3 父格網。")
...
def get_resolution(cell: str) -> int:
    """取得 H3 cell 的解析度。"""
    if h3 is None:
        raise RuntimeError("缺少 h3 套件，無法取得 H3 解析度。")
