...
2. **Config**：硬編 6 類正準順序（`CLASSES_EN/ZH` + `MEDIC_MAP`，對齊 `utils/config.py`）；超參 `BATCH=64 / LR=1e-3 / Adam / SEED=42 / AMP=on / EPOCHS=30`；`MAX_SAMPLES_PER_CLASS`、`MAX_EVAL_PER_CLASS`（可選每類抽樣上限，控時間）；`CACHE_IN_MEMORY=True`（影像解碼一次存記憶體，提高 GPU 使用率，RAM 不足時設 False）；`ImageFile.LOAD_TRUNCATED_IMAGES=True`。
3. imports。
...
8. DataLoader：train 用 `WeightedRandomSampler`（**sqrt-inverse 類別權重**，沿用 Huang 思路處理不平衡）；dev/test 不抽樣。`pin_memory=True`（加速資料載入，這是真正瓶頸）。`cache=True`（即 `CACHE_IN_MEMORY=True`）時用 `num_workers=0`（避免每個 worker 複製整份記憶體快取）；否則開 `num_workers=CPU 核數`、`persistent_workers=True`、`prefetch_factor=4`（避免 worker 每 epoch 重生）。
9. **v1 模型定義**：與 `models/custom_cnn_model.py::DisasterCNN_v1` **逐位元組一致**，`num_classes=6`（GAP 設計，換類別只動最後一層 `Linear(256,6)`）。
10. `train_one_model` + 訓練 v1：**用 MEDIC `dev` 當驗證**、best-by-dev-acc、AMP；`loss_fn = CrossEntropyLoss(label_smoothing=0.1)`（減少過擬合）；optimizer 後接 `CosineAnnealingLR(T_max=epochs)`（每 epoch 末呼叫 `scheduler.step()`，cosine 衰退 LR 有助後期收斂）。
...
- notebook 末尾新增「ResNet18 遷移學習 vs 自訓 CNN」比較節（報告用，載入上傳的 custom_cnn.pth 對比，不部署進 app）。
