...
2. **Config**：硬編 6 類正準順序（`CLASSES_EN/ZH` + `MEDIC_MAP`，對齊 `utils/config.py`）；超參 `BATCH=64 / LR=1e-3 / Adam / SEED=42 / AMP=on`；`MAX_SAMPLES_PER_CLASS`、`MAX_EVAL_PER_CLASS`（可選每類抽樣上限，控時間）；`ImageFile.LOAD_TRUNCATED_IMAGES=True`。
3. imports。
...
8. DataLoader：train 用 `WeightedRandomSampler`（**sqrt-inverse 類別權重**，沿用 Huang 思路處理不平衡）；dev/test 不抽樣。`num_workers=CPU 核數`、`pin_memory=True`（加速資料載入，這是真正瓶頸）。loader 設 `persistent_workers=True`、`prefetch_factor=4`（避免 worker 每 epoch 重生）。
